...
class Submissions extends Metric
{
    ...
    public function filterMetricQueryData($payload = [])
    {
        ...
        $payload['join']['left join ' . $configuration['submissions_meta_table'] . ' as sm'] = ' ON sm.submission_id = bl.id';
        $payload['join']['left join ' . $configuration['logs_table'] . ' as l'] = ' ON l.id = sm.meta_value';

        $payload['where']['sm.meta_key'] = ' = "box_log_id"';

        // Search box
        if (isset($payload['where']['bl.box']))
        {
            $payload['where']['l.box'] = $payload['where']['bl.box'];
        }

        // Search country
        if (isset($payload['where']['bl.country']))
        {
            $payload['where']['l.country'] = $payload['where']['bl.country'];
        }

        // Search device
        if (isset($payload['where']['bl.device']))
        {
            $payload['where']['l.device'] = $payload['where']['bl.device'];
        }

        // Search event
        if (isset($payload['where']['bl.event']))
        {
            $payload['where']['l.event'] = $payload['where']['bl.event'];
        }

        // Search page
        if (isset($payload['where']['bl.page']))
        {
            $payload['where']['l.page'] = $payload['where']['bl.page'];
        }
